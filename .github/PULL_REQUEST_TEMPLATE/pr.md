...
<td>9</td>
<td>Have additional APIs been written e.g. C overloads in <code>aoclda_c_wrapper.cpp</code>, Python API and Scikit-learn patch? Do these APIs have tests and examples where appropriate?</td>
</tr>
...
<td>10</td>
<td>
If this PR adds or modifies member variables or options in an algorithm handle:<br>

 * If new mandatory variables have been added, are they correctly included in the algorithm's <code>serialize</code> function?
 * Has the algorithm's <code>serialization_version</code> been updated in its constructor if the change introduces a breaking change to that algorithm's serialization (e.g., new/removed serialized fields, changed serialization order, modified option enums for that algorithm)?
 * If this introduces a <b>global</b> breaking change to model persistence infrastructure (e.g., metadata format, common base class fields): has <code>model_persistence_min_version</code> been bumped in <code>model_persistence.hpp</code>?
...
Are all array-type objects (vectors, etc) that are dynamically allocated (new, resized, etc) properly ringfenced with `try`-`catch` and return on error `da_status_memory_error`? Also, are all `new` instances `delete`d? Are all `new[]` objects properly `delete[]`d? Are large integer multiplations e.g. `n_features x n_samples` allocation guarded from LP64 overflow e.g. by casting to size_t?</td>
</tr>
<tr>
<td>
...
<td>39</td>
<td>If the PR adds new member variables or modifies data structures in an algorithm handle: are the new variables correctly serialized? If this constitutes a breaking change, has the developer correctly bumped <code>model_persistence_min_version</code> (global) or the algorithm's <code>serialization_version</code> (algorithm-specific) as appropriate?</td>
</tr>
</table>
...
<td>51</td>
<td>Is the code coverage by the unit tests sufficient? Particularly, are all possible code paths and functionality use cases exercised? (e.g., different AVX extensions, types of matrices, row-major and column-major data, a test that checks NaN data doesn't produce segfaults)</td>
</tr>
<tr>
<td>
