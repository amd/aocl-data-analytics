 ...
 *
 * The routines all take a \ref da_datastore object as their first argument, which must be initialized prior to the routine call using \ref da_datastore_init.
 * 
 * \subsection option_setting Option setting
 * Prior to reading the CSV file, various options can be set by passing the \ref da_datastore to \ref da_options_set_int or \ref da_options_set_string for integer or string options respectively.
 * 
 ...
 * \param[out] headings a pointer to the ncols array of strings containing the column headings. If the option CSV use header row is set to 0 (the default) then this argument is not used. Otherwise, note that this routine allocates memory for \a headings internally. It is your responsibility to deallocate this memory.
 ...
 * - \ref da_status_bad_lines - the CSV file contained one or more lines that could not be read
 * - \ref da_status_missing_data - the returned array contains missing data
 * - \ref da_status_no_data - no data was found
 */
...
/** \{
 * \brief Read char string data from a CSV file, together with a header row containing the column labels.
 * 
