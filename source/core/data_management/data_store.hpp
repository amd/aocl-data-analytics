...
class data_store {
  ...
  public:
    ...
    // FIXME this should be a specialization in a separate file
    da_status concatenate_cols_csv(da_int mc, da_int nc, char ***data, da_ordering order,
                                   bool copy_data, bool C_data) {
    ...
    da_status raw_ptr_from_csv_columns([[maybe_unused]] da_csv::csv_reader *csv,
                                       da_auto_detect::CSVColumnsType &columns,
                                       da_int start_column, da_int end_column,
                                       da_int nrows, char ****bl, bool &C_data) {
        ...
        for (da_int i = 0; i < ncols; i++) {
            if (std::vector<char **> *char_col =
                ...
                for (da_int j = 0; j < nrows; j++) {
                    da_status tmp_error = da_csv::char_to_num(
                        parser, *(*char_col)[j], &p_end, &b[i * nrows + j], maybe_int);
